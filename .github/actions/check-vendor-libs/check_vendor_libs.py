...
def check_path(
    path: Path, origins: dict[str, dict[str, typing.Any]] | None = None
) -> bool:
    """
...
def compare(entry: Path, origin: dict[str, str]) -> bool:
    ...
    if entry.is_dir():
        ...
        for dir_entry in entry.iterdir():
            new_fragment = str(
                Path(purl.fragment) / (dir_entry.name if dir_entry.is_dir() else "")
            )
            result &= compare(
                dir_entry,
                dict(
...
def get_download_url(purl: urllib.parse.ParseResult) -> str:
    ...
    if not download_url:
        ...
        download_url = {
            "github": f"https://github.com/{purl_namespace}/{purl_name}/archive/{purl_version}.tar.gz",
            "npm": f"https://registry.npmjs.org/{purl_namespace}{purl_namespace and '/' or ''}{purl_name}/-/{purl_name}-{purl_version}.tgz",
        }.get(purl_type) or ""
    return download_url
...
def __main__():
    ...
    parser.add_argument(
        "--no-error", action="store_true", help="do not exit with an errorlevel"
    )
